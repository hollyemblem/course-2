...
us12 %>% group_by(response) %>% summarise(subtotal = n()) %>% ungroup() %>% mutate(proportion = subtotal / sum(subtotal))
...
A margin of error allows us to calculate the parameters of how many atheists are in the US, within a given range.
...
margin_error <-1.96* sqrt(0.0499*(1-0.0499)/1002 )
```
...
it12 <- atheism %>% filter(nationality == "Italy" , atheism$year == "2012")
it12 %>% group_by(response) %>% summarise(subtotal = n()) %>% ungroup() %>% mutate(proportion = subtotal / sum(subtotal))
inference(y = response, data = it12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")

fr12 <- atheism %>% filter(nationality == "France" , atheism$year == "2012")
fr12 %>% group_by(response) %>% summarise(subtotal = n()) %>% ungroup() %>% mutate(proportion = subtotal / sum(subtotal))
inference(y = response, data = fr12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
...
<li> The most conservative estimate when calculating a confidence interval occurs when $p$ is set to 1. -FALSE </li>
</ol>
...
inference(y = response, x=year, data = spain, statistic = "proportion", type = "ht", method = "theoretical", success = "atheist", alternative="two sided", null = 0)

```


We can also use this too though, which is arguably easier: 

```{r}
prop.test(c(36,16),c(79,78), alternative="greater")
```
...
inference(y = response, x=year, data = us, statistic = "proportion", type = "ht", method = "theoretical", success = "atheist", alternative="two sided", null = 0)
