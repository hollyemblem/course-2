...
<li> 50\% of houses in Ames are smaller than 1,499.69 square feet. </li> < - This one
...
```{r samp1-dist}
samp1 %>% summarise(x_bar = mean(area),med = median(area), sigma = sd(area), pop_iqr = IQR(area), pop_min = min(area),pop_max = max(area),pop_q1 = quantile(area, 0.25), pop_q3 = quantile(area, 0.75))
...
<li> Sample size of 1000. <- This one </li>
</ol>
...
<ol>
<li> Each element represents a mean square footage from a simple random sample of 10 houses. </li> <- this one
...
```{r price-sample}
samp1 <- ames %>% summarise(x_bar_mean = mean(price))
...
sample_means15 <- ames %>% rep_sample_n(size = 15, reps = 1, replace = TRUE) %>% summarise(x_bar = mean(price))
...
sample_means15 <- ames %>% rep_sample_n(size = 15, reps = 2000, replace = TRUE) %>% summarise(x_bar = mean(price))
...
sample_means150 <- ames %>% rep_sample_n(size = 150, reps = 2000, replace = TRUE) %>% summarise(x_bar = mean(price))
ggplot(data = sample_means150, aes(x = x_bar)) + geom_histogram(binwidth = 20)
...
<ol>
<li> The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`). </li> <- This one
