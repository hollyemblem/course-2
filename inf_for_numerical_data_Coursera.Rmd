...
```{r summary}
summary(nc$gained)
 ggplot(data=subset(nc, !is.na(gained)), aes(x=gained)) + geom_histogram(binwidth=0.5)
```
...
mybox <<-ggplot(data=nc, aes(y=weight, x=habit, fill-habit)) 
mybox <- mybox + geom_boxplot()
mybox
```
...
We are satisified that the data is independent
Is not heavily skewed
Is not greater than 10% of the population
</div>
...
<li> We are 95% confident that the difference in average weights of babies whose moms are smokers and nonsmokers is between 0.05 to 0.58 pounds. - This one </li>
...
```{r weeks-ci-99}
inference(y = weeks, conf_level = 0.99, data = nc, statistic = "mean", type = "ci", null = 0,  alternative = "twosided", method = "theoretical")
...
inference(y = weeks, conf_level = 0.90, data = nc, statistic = "mean", type = "ci", null = 0,  alternative = "twosided", method = "theoretical")
```
...
ggplot(data=subset(nc, !is.na(mature)), aes(x=mage)) + geom_histogram(binwidth=0.5)
nc %>% group_by(mature) %>% summarise(mean_age = mean(mage), median_age = median(mage),max_age=max(mage), min_age=min(mage))
```
I create a histogram of the distribution of age, then calculate summary statistics
...
your project as well.)
 ...
 $H_0: \mu_{mature} = \mu_{younger}$; $H_A: \mu_{mature} \ne \mu_{younger}
Hypothesis test to see if the weight of babies to younger moms is different to mature moms.
```{r pick-your-own}
